...
5. Despliega: `npm run deploy` desde `/worker` — te da la URL `https://asistentecarmen.TU-SUBDOMINIO.workers.dev`
...
2. Registra el webhook (una sola vez, después de desplegar el Worker):
   ```
   curl "https://api.telegram.org/bot<TOKEN>/setWebhook?url=https://asistentecarmen.TU-SUBDOMINIO.workers.dev/telegram"
   ```

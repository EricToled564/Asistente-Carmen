export default function NavTabs({ tabs, active, onChange }) {
  return (
    <nav className="safe-bottom border-t border-terracota-100 bg-crema-50/95 backdrop-blur">
            ...
            <li key={key} className="flex-1">
              <button
                ...
                className={`flex w-full flex-col items-center gap-0.5 py-2.5 text-xs font-medium transition-colors ${
                  isActive ? 'text-terracota-600' : 'text-noche-900/40'
                }`}
                aria-current={isActive ? 'page' : undefined}
              >
                <span className={`text-xl leading-none ${key === 'sos' ? '' : ''}`}>{tab.icon}</span>
                <span>{tab.label}</span>
                {isActive && <span className="mt-0.5 h-1 w-1 rounded-full bg-terracota-500" />}
              </button>
            </li>
          )
        })}

...
export default function SOS() {
  ...
  async function disparar() {
    ...
    if (config.whatsappNumero && posicion.ok) {
      const texto = encodeURIComponent(
        `🆘 SOS desde Nava. Mi ubicación: https://www.google.com/maps?q=${posicion.lat},${posicion.lng}${
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-6 p-6 text-center">
      <div>
        <h1 className="font-display text-2xl font-semibold text-terracota-700">SOS</h1>
        <p className="mt-1 max-w-xs text-sm text-noche-900/60">
      ...
      {estado === 'enviando' && <p className="text-sm text-noche-900/60">Enviando…</p>}
      {estado === 'enviado' && <p className="text-sm text-terracota-700">Aviso enviado ✓</p>}
      {estado === 'error' && (
      ...
      <p className="max-w-xs text-xs text-noche-900/40">

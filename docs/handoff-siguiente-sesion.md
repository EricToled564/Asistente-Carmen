...
**Vía API — contrato ya verificado:**

```bash
export ELEVENLABS_API_KEY=...        # elevenlabs.io -> perfil -> API Keys
export WORKER_URL=https://asistentecarmen.erictoled564.workers.dev

node elevenlabs/registrar-webhooks.mjs --dry-run    # imprime los 8 payloads, no llama a nadie
node elevenlabs/registrar-webhooks.mjs              # los registra de verdad
node elevenlabs/registrar-webhooks.mjs --verificar   # lista cómo quedó
```

El contrato **ya no hace falta comprobarlo**: se verificó contra los tipos del SDK oficial
`@elevenlabs/elevenlabs-js` (bajado con `npm pack`, que sí es alcanzable aunque elevenlabs.io esté
bloqueado). `WebhookToolConfigInput`, `WebhookToolApiSchemaConfigInput` y `PromptAgentApiModelInput`
confirman `POST /v1/convai/tools` con `{tool_config}`, el `api_schema` con `query_params_schema` /
`request_body_schema`, y el enganche en `conversation_config.agent.prompt.tool_ids` vía PATCH del
agente. Header `xi-api-key`.

Había un segundo script, `worker/scripts/registrar-tools-elevenlabs.mjs`, escrito a ciegas antes de
esa verificación. Su contrato resultó ser correcto, pero se borró en favor de este porque le
faltaban tres cosas que sí importan: no era idempotente (cada corrida creaba 8 tools nuevas, no las
actualizaba), pisaba `tool_ids` en vez de hacer la unión con lo que el agente ya tuviera, y no
respaldaba al agente antes del PATCH.

El de `elevenlabs/` sí: busca por nombre y actualiza si ya existe, hace la unión de tool_ids, guarda
la config del agente en un `.backup-agente-*.json` local antes de tocarlo, y al terminar relee para
comprobar que el system prompt sigue teniendo caracteres (si el PATCH se lo llevara, avisa y sale
con código 1 en vez de dejar a Maite muda). Ver `elevenlabs/README.md`.
